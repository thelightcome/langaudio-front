<template>
  <div v-click-outside="close" class="relative block w-full">
    <div class="w-full" @click="toggle()">
      <span v-show="props.title" class="block mb-2 text-md text-light">{{
        props.title
      }}</span>
      <button
        class="w-full box-border border-light text-light px-3 py-1 border cursor-pointer rounded-md duration-300"
    ...
    <transition name="slide-top">
      <div
        v-if="state"
        class="absolute left-0 top-full bg-black text-light border-light border mt-1 min-w-full pb-4 duration-300 min-h-[150px] z-10"
      >
        <UiScrollLoading
          :loading="loading"
          :can-load="canLoad"
